...
public class HeadTiltPlushItem extends GenericPlushItem {
    ...
    public TypedActionResult<ItemStack> use(World world, PlayerEntity user, Hand hand) {
        ...
        if (nbt == null || !nbt.contains(AshbornModNbtKeys.TILT)) {
            ...
            nbt.putString(AshbornModNbtKeys.TILT, HeadTiltPlushBlock.State.DEFAULT.asString());
        }
        HeadTiltPlushBlock.State tilt = HeadTiltPlushBlock.State.fromName(nbt.getString(AshbornModNbtKeys.TILT));
        if (!user.isSneaking()) return super.use(world, user, hand);
        if (!world.isClient()) {
            stack.getOrCreateNbt().putString(AshbornModNbtKeys.TILT, tilt.getNext().asString());
