...
public class DryadCloakItemRenderer implements BuiltinItemRendererRegistry.DynamicItemRenderer {
    private static final Identifier TEXTURE = AshbornMod.getId("textures/entity/cloak_dryad.png");

    @Nullable
    private DryadCloakModel<PlayerEntity> model;

    @Override
    public void render(ItemStack stack, ModelTransformationMode mode, MatrixStack matrices, VertexConsumerProvider vertexConsumers, int light, int overlay) {
        if (model == null) {
            this.model = new DryadCloakModel<>(DryadCloakModel.getTexturedModelData().createModel());
        }
