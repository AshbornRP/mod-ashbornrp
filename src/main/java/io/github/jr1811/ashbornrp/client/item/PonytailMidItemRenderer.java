...
public class PonytailMidItemRenderer implements BuiltinItemRendererRegistry.DynamicItemRenderer {
    private static final Identifier TEXTURE = AshbornMod.getId("textures/entity/ponytail_mid.png");

    @Nullable
    private PonytailMidModel<PlayerEntity> model;

    @Override
    public void render(ItemStack stack, ModelTransformationMode mode, MatrixStack matrices, VertexConsumerProvider vertexConsumers, int light, int overlay) {
        if (model == null) {
            this.model = new PonytailMidModel<>(PonytailMidModel.getTexturedModelData().createModel());
        }
