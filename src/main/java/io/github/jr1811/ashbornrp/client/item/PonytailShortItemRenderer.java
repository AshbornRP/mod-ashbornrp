...
public class PonytailShortItemRenderer implements BuiltinItemRendererRegistry.DynamicItemRenderer {
    private static final Identifier TEXTURE = AshbornMod.getId("textures/entity/ponytail_short.png");

    @Nullable
    private PonytailShortModel<PlayerEntity> model;

    @Override
    public void render(ItemStack stack, ModelTransformationMode mode, MatrixStack matrices, VertexConsumerProvider vertexConsumers, int light, int overlay) {
        if (model == null) {
            this.model = new PonytailShortModel<>(PonytailShortModel.getTexturedModelData().createModel());
        }
