...
public record AccessoryEntryColors(LinkedList<Integer> indexedColors) {
    ...
    public static void clearStack(ItemStack stack) {
        ...
        if (nbt != null) nbt.remove(AshbornModNbtKeys.ACCESSORY_COLORS_LEGACY);
    }
    ...
    public static boolean hasColor(ItemStack stack) {
        return stack.getNbt() != null && stack.getNbt().contains(AshbornModNbtKeys.ACCESSORY_COLORS_LEGACY);
    }
    ...
    public static AccessoryEntryColors fromNbt(@Nullable NbtCompound nbt) {
        if (nbt == null || !nbt.contains(AshbornModNbtKeys.ACCESSORY_COLORS_LEGACY)) {
            return null;
        }
        NbtList colorNbt = nbt.getList(AshbornModNbtKeys.ACCESSORY_COLORS_LEGACY, NbtElement.INT_TYPE);
        if (colorNbt.isEmpty()) {
            return null;
        }
    ...
    public void toNbt(NbtCompound nbt, boolean clearPrevious) {
        NbtList colorNbt = nbt.contains(AshbornModNbtKeys.ACCESSORY_COLORS_LEGACY) && !clearPrevious ? nbt.getList(AshbornModNbtKeys.ACCESSORY_COLORS_LEGACY, NbtElement.INT_TYPE) : new NbtList();
        this.indexedColors.forEach(color -> colorNbt.add(NbtInt.of(color)));
        nbt.put(AshbornModNbtKeys.ACCESSORY_COLORS_LEGACY, colorNbt);
    }
